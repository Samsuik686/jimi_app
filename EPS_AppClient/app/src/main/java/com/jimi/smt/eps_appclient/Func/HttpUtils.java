...
import com.jimi.smt.eps_appclient.Beans.NewOperateBean;
import com.jimi.smt.eps_appclient.Beans.Operate;
...
public class HttpUtils {
    ...
    private static final String OPERATE_URL = "/program/operate";
    ...
    public static final int CodeMergeOperate = 17;
...
//                        Log.d(TAG, "getMaterials - onResponse - " + response);
    ...
    public void addWareOperation(String materialId,String cycle,Material.MaterialBean bean,NewOperateBean operation, ArrayList<ArrayList<Integer>> integerLists) {
        Log.d(TAG, "addOperation - " + OPERATE_URL);
        operation.setCycle(cycle);
        operation.setMaterialId(materialId);
        Gson gson = new Gson();
        String operationStr = gson.toJson(operation);
        String operateStr1 = gson.toJson(bean);
        Log.d(TAG, "addOperation - " + operationStr);
        Log.d("sxmmm", "operateStr1 - " + operateStr1);
        Log.d("sxmmmmm", "addOperation - " + operationStr);
        OkHttpUtils.postString().url(globalData.getIp() + OPERATE_URL)
                .content(operationStr)
                .mediaType(MediaType.parse("application/json;charset=utf-8"))
                .addHeader("dataType", "json")
                .build().connTimeOut(Constants.CONNECT_TIMEOUT).writeTimeOut(Constants.WRITE_TIMEOUT).readTimeOut(Constants.READ_TIMEOUT)
                .execute(new StringCallback() {
                    @Override
                    public void onError(Call call, Exception e, int id) {
                        Log.d("sxmmm","addOperation - onError - " + e.toString());
                        Log.d(TAG, "addOperation - onError - " + e.toString());
                        mOkHttpInterface.showHttpError(CodeAddOperate, new Object[]{bean,operationStr, integerLists}, e.toString());
                    }

                    @Override
                    public void onResponse(String response, int id) {
                        Log.d(TAG, "addOperation - onResponse - " + response);
                        Log.d("sxmmm","addOperation - onResponse - " + response);
                        mOkHttpInterface.showHttpResponse(CodeAddOperate, new Object[]{bean,operationStr, integerLists}, response);
                    }
                });
     ...
     * @param visit
     */
    public void updateWareVisit(Material.MaterialBean bean, ProgramItemVisit visit, int count, ArrayList<ArrayList<Integer>> integerLists) {
        ...
        OkHttpUtils.postString().url(globalData.getIp() + addVisit)
                ...
                .execute(new StringCallback() {
                    ...
                    public void onError(Call call, Exception e, int id) {
                        ...
                        mOkHttpInterface.showHttpError(CodeAddVisit, new Object[]{bean, visit, count, integerLists}, e.toString());
                    }

                    @Override
                    public void onResponse(String response, int id) {
                        Log.d(TAG, "updateVisit - onResponse - " + response);
                        mOkHttpInterface.showHttpResponse(CodeAddVisit, new Object[]{bean, visit, count, integerLists}, response);
                    }
                });
    }
     ...
     * @param integerLists
     */
    //String materialId,String cycle,
    public void mergeOperate(String materialId,String cycle,Material.MaterialBean bean, Operate operate, int count, ArrayList<ArrayList<Integer>> integerLists, ArrayList<Integer> integers) {
        Log.d(TAG, "mergeOperate - " + OPERATE_URL);
        operate.setCycle(cycle);
        operate.setMaterialId(materialId);
        Gson gson = new Gson();
        String operateStr = gson.toJson(operate);
        String operateStr1 = gson.toJson(bean);
        Log.d(TAG, "operateStr1 - " + operateStr1);
        Log.d(TAG, "operateStr - " + operateStr);
        Log.d("sxmmm", "operateStr1 - " + operateStr1);
        Log.d("sxmmm", "operateStr - " + operateStr);
        OkHttpUtils.postString().url(globalData.getIp() + OPERATE_URL)
                .content(operateStr)
                .mediaType(MediaType.parse("application/json;charset=utf-8"))
                .build()/*.connTimeOut(Constants.CONNECT_TIMEOUT).writeTimeOut(Constants.WRITE_TIMEOUT).readTimeOut(Constants.READ_TIMEOUT)*/
                .execute(new StringCallback() {
                    @Override
                    public void onError(Call call, Exception e, int id) {
                        Log.d(TAG, "mergeOperate - onError - " + e.toString());
                        mOkHttpInterface.showHttpError(CodeMergeOperate, new Object[]{bean, operate, count, integerLists, integers}, e.toString());
                        Log.d("sxmmm","onError - " + e.toString());
                    }

                    @Override
                    public void onResponse(String response, int id) {
                        Log.d(TAG, "mergeOperate - onResponse - " + response);
                        Log.d("sxmmm","response=="+response);
//                        response = "{\"code\":0,\"msg\":\"操作失败\"}";
                        mOkHttpInterface.showHttpResponse(CodeMergeOperate, new Object[]{bean, operate, count, integerLists, integers}, response);
                    }
                });
    }
