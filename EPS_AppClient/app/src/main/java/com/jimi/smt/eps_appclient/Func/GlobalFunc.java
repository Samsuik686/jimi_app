...
public class GlobalFunc {
    ...
    //根据扫的站位条码获取站位值 (两者兼容100805118,3040101)3021007   SMT030101
    public String getLineSeat(String line,String scanValue) {
        scanValue = scanValue.replaceAll(" ", "").trim();
        String linSeat = scanValue;
        if (scanValue.toUpperCase().contains(line)){
            linSeat = scanValue.substring(line.length());
            linSeat = linSeat.substring(0,2)+"-"+linSeat.substring(2,linSeat.length());
            return linSeat;
        }
        if (scanValue.length() >= 8) {
