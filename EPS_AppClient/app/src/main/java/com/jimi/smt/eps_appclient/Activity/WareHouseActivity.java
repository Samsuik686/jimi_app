...
import com.jimi.smt.eps_appclient.Func.NetSpeed;
import com.jimi.smt.eps_appclient.Func.NetSpeedTimer;
            ...
            switch (msg.what) {
                ...
                case NetSpeedTimer.NET_SPEED_TIMER_DEFAULT:
                    ...
                    if (loadingDialog != null && loadingDialog.isShowing()) {
                        loadingDialog.setMsgWithSpeed("正在加载... " + speed + " (kb/s)");
                    }
//                    Log.d(TAG,"speed - " + speed);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //获取网速
        mNetSpeedTimer = new NetSpeedTimer(this, new NetSpeed(), mWareHandler)
                .setDelayTime(500)
                .setPeriodTime(500);
                ...
                case KeyEvent.ACTION_UP:
                    ...
                    if (globalFunc.isNetWorkConnected()) {
                        ...
                        if (!TextUtils.isEmpty(v.getText().toString().trim())) {
                            ...
                            try {
                                temp=scanMaterial.split("@");
                                name = temp[0];
                                materialId = temp[2];
                                cycle = temp[7];
                            }catch (Exception e){
//                                Toast.makeText(WareHouseActivity.this, "二维码有错", Toast.LENGTH_LONG).show();
                            }
                            ...
                            switch (v.getId()) {
                                case R.id.et_ware_scan_material:
                                    ...
                                    if (scanMaterial.length() <= Constants.MATERIAL_LENGTH) {
                                        ...
                                        //写日志
                                        showLoading();
                                        setOperateLog(arrayLists, lineSeatList, scanMaterial);
                                    } else {
    ...
    //写日志并显示 
    private void setOperateLog(ArrayList<ArrayList<Integer>> integerLists, ArrayList<String> lineSeatList, String scanMaterial) {
        wareCount = -1;
        if (integerLists != null) {
            if (integerLists.size() > 0) {
                ArrayList<ArrayList<Integer>> arrayList = new ArrayList<ArrayList<Integer>>(/*integerLists*/);

                arrayList.addAll(integerLists);

                wareCount = arrayList.size();
                Log.d(TAG, "setOperateLog::" + wareCount);
                ArrayList<Integer> lineSeatIndex = new ArrayList<>();
                Material.MaterialBean operateBean = new Material.MaterialBean();
                for (int m = 0; m < wareCount; m++) {
                    lineSeatIndex.clear();
                    lineSeatIndex.addAll(arrayList.get(m));
                    ...
                    mHttpUtils.updateWareVisit(operateBean,itemVisit,m,arrayList);
                    //添加日志
                    Operation operation = Operation.getOperation(curOperatorNUm, Constants.STORE_ISSUE, operateBean);
                    mHttpUtils.addWareOperation(operation,arrayList);
                    */
                }

                //更新visit、添加日志
                Operate operate = Operate.getOperateObj(curOperatorNUm, Constants.STORE_ISSUE, lineSeatList, operateBean);
//                mHttpUtils.mergeOperate(materialId,cycle,operateBean, operate, wareCount, arrayLists,null);
                mHttpUtils.mergeOperate(materialId,cycle,operateBean, operate, wareCount, arrayLists,null);
...
//
            Material.MaterialBean bean = new Material.MaterialBean(globalData.getWork_order(), globalData.getBoard_type(), globalData.getLine(), globalData.getProgramId(),
                    -1, false, "", "", -1, "", "", "-1", "", "FAIL", "不存在该料号的站位!");

            NewOperateBean operation = NewOperateBean.getOperateObj(curOperatorNUm, 4,null, bean);
//            mHttpUtils.addOperation(operation);
            String[] lineseat = new String[0];
            operation.setMaterialNo(name);
            operation.setStoreIssueResult(1);
            operation.setProgramId(globalData.getProgramId());
            operation.setLineseat(lineseat);
//            mHttpUtils.addWareOperation(materialId,cycle,bean,operation,integerLists);
            mHttpUtils.addWareOperation(materialId,cycle,bean,operation,integerLists);
...
////            mHttpUtils.addOperation(operation);
//            mHttpUtils.addWareOperation(operation,integerLists);
    ...
    private void clearDisplay(ArrayList<ArrayList<Integer>> integerLists) {
        ...
        for (int k = 0, len = lineSeatIndex.size(); k < len; k++) {
            ...
            bean.setScanlineseat("");
            bean.setScanMaterial("");
            Log.d(TAG, bean.getMaterialNo() + " - result - " + bean.getResult());
        }
        //刷新数据
        wareHouseAdapter.notifyDataSetChanged();
        et_ware_scan_material.requestFocus();
    ...
    public void showHttpResponse(int code, Object request, String response) {
        ...
        switch (code) {
            ...
            case HttpUtils.CodeMergeOperate:
                dismissLoading();
                if (resCode >= 1) {
                    //更新成功,显示并写本地数据
                    showInfo("料号:" + scanMaterial, lineSeatAl, wareSeatList, 1);
                    //保存本地数据库缓存
                    cacheWareResult((ArrayList<ArrayList<Integer>>) (((Object[]) request)[3]));
                    //刷新数据
                    ...
                    clearDisplay((ArrayList<ArrayList<Integer>>) (((Object[]) request)[3]));
                }
                break;
        }
    }
    ...
    public void showHttpError(int code, Object request, String s) {
        dismissLoading();
        globalFunc.showInfo("警告", s, "请检查!");
        Log.d(TAG, "showHttpError - " + code + " - " + s);
        switch (code) {
            case HttpUtils.CodeMergeOperate:
                //取消料号弹出框
                if (null != infoDialog && infoDialog.isShowing()) {
                    infoDialog.dismiss();
                }
                //更新失败,网络访问失败
                clearDisplay((ArrayList<ArrayList<Integer>>) (((Object[]) request)[3]));
